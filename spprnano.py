...
# Используются для начального предположения: M_E или M_S.
# Исследователь может переопределить в таблице.
#
# Правило назначения:
#   M_E — маркер эффективности: его РОСТ относительно контроля желателен.
#          Примеры: SOD-Zn, CA4, ADH1B, ZAG, MT, Zn, I, белок.
#   M_S — маркер безопасности: его РОСТ относительно контроля нежелателен (риск).
#          Примеры: Pb, Cd, As, Al, Ni, Sn, V, Li, B, Si.
#   Один маркер — одна роль. Дублирование недопустимо.
# ======================
MS_HINTS = [
    # Токсичные элементы
...
def guess_role(col_name: str) -> str:
    """
    Начальное предположение о роли маркера по имени столбца.
    Возвращает 'M_S (безопасность/риск)' или 'M_E (эффективность)'.
    Исследователь может изменить в таблице.
    """
...
def compute_efficiency(
    ...
    Для каждого маркера m и группы g:
        shift_m = (median(g, m) − median(ctrl, m)) / IQR(all, m)

    shift_m > 0 → маркер вырос относительно контроля → хорошо для M_E.
    shift_m < 0 → маркер снизился → плохо, но НЕ штрафуется в S_g
                  (снижение маркера эффективности — это не риск безопасности).

    E_g = nanmedian(shift_m) по всем маркерам m ∈ M_E.
    """
    ...
    for group in df[group_col].unique():
        ...
        for m in me_markers:
            ...
            # Положительный сдвиг = маркер вырос = хорошо для M_E
...
def compute_safety(
    ...
    """
    S_g = медиана рисков по маркерам безопасности.

    Для каждого маркера m и группы g:
        shift_m = (median(g, m) − median(ctrl, m)) / IQR(all, m)
        risk_m  = max(0, shift_m)

    shift_m > 0 → токсичный маркер вырос → риск, учитываем.
    shift_m < 0 → токсичный маркер снизился → риска нет, risk_m = 0.
                  Снижение токсиканта НЕ даёт бонус к эффективности —
                  это просто отсутствие вреда.

    S_g = nanmedian(risk_m) по всем маркерам m ∈ M_S.
    S_g всегда ≥ 0.
    """
    ctrl_data = df[df[group_col] == control_group]
    S_vals = {}
    ...
    for group in df[group_col].unique():
        ...
        for m in ms_markers:
            ...
            shift = (g_vals.median() - c_vals.median()) / iqr_m
            risk = max(0.0, shift)   # только рост токсиканта — это риск
            risks.append(risk)
